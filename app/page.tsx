  ...
  async (page: number, pageSize: number) => {
    try {
      ...
      const editions: Edition[] = editionsRaw.map((edition: any) => ({
        ...
        updatedAt: edition.updatedAt ? new Date(edition.updatedAt).toISOString() : undefined,
      }));
...
const getProxyUrl = (rawUrl: string, edition?: Edition) => {
  if (!rawUrl) return '';
  const v = edition?.updatedAt ? `v=${new Date(edition.updatedAt).getTime()}` : '';
  let url = rawUrl;
  if (v) {
    url = `${rawUrl}${rawUrl.includes('?') ? '&' : '?'}${v}`;
  }
  return url;
};
                  ...
                  >
                    ...
                    <div className="relative aspect-[2/3] w-full overflow-hidden bg-gray-100" suppressHydrationWarning>
                      {edition.pages && edition.pages[0]?.url ? (
                        <Image
                          src={getProxyUrl(edition.pages[0].previewUrl || edition.pages[0].url, edition)}
                          alt={edition.name}
                          fill
